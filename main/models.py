...
class User(AbstractUser):
    bioText = models.CharField(max_length=100, editable=True)
    imageURL = models.ImageField(editable=True)
    pass

class Review(models.Model):
    id = models.AutoField(primary_key=True)
    dateCreated = models.DateField(auto_now_add=True, editable=True)
    textField = models.CharField(max_length=1000, editable=True)
    imageURL = models.ImageField(editable=True)
...
class Comment(models.Model):
    id = models.AutoField(primary_key=True)
    dateCreated = models.DateField(auto_now_add=True, editable=True)
    textField = models.CharField(max_length=300, editable=True)
